...
//                 if (!membersMap.has(assessment.name)) {
//                     membersMap.set(assessment.name, { totalScoreSum: 0, count: 0, details: [] });
//                 }
//                 const memberData = membersMap.get(assessment.name);
//                 const detailScore = assessment.detail_score ? Number(Number(assessment.detail_score).toFixed(1)) : 0;
//                 memberData.totalScoreSum += detailScore; // Sum of detail_scores which are already averages per review
//                 memberData.count++;
//                 memberData.details.push({
//                     reviewer: assessment.reviewer,
//                     score: detailScore,
//                     comment: assessment.comment || 'Không có bình luận'
//                 });
//             }

//             for (const [name, data] of membersMap) {
//                 const avgScore = data.count > 0 ? data.totalScoreSum / data.count : 0;
//                 peerReviewData.push({
//                     name: name,
