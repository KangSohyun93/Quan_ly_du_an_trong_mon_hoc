...
dotenv.config({ path: "./.env.github" }); // Đảm bảo đường dẫn này đúng
...
const fetchAndStoreCommits = async (projectId) => {
  ...
  try {
    ...
    if (!project || !project.github_repo_url) {
      console.error(
        `[GitHubService] Project ${projectId} not found or has no github_repo_url.`
      );
      return {
        success: false,
        message: `Project ${projectId} not found or has no github_repo_url.`,
        count: 0,
        data: [],
      };
    }
    ...
    if (parts.length < 5 || !repoUrl.startsWith("https://github.com/")) {
      ...
      return {
        success: false,
        message: `Invalid repoUrl format: ${repoUrl}`,
        count: 0,
        data: [],
      };
    }
    ...
    for (const branch of branchesResponse.data) {
      console.log(
        `[GitHubService] Fetching commits for branch: ${branch.name}`
      );
      const commitsListResponse = await octokit.request(
      ...
      console.log(
        `[GitHubService] Found ${commitsListResponse.data.length} commits on branch ${branch.name}. Fetching details...`
      );

      for (const commitListItem of commitsListResponse.data) {
        try {
          const detailedCommitResponse = await octokit.request(
          ...
          allCommitsDataFromGitHub.push({
            ...
            author_email:
              detailedCommit.commit.author?.email || "unknown@example.com",
            date:
              detailedCommit.commit.author?.date || new Date().toISOString(),
          ...
          console.warn(
            `[GitHubService] Could not fetch details for commit ${commitListItem.sha}: ${detailError.message}. Storing with 0 lines changed.`
          );
          allCommitsDataFromGitHub.push({
            sha: commitListItem.sha,
            message: commitListItem.commit.message,
            author_email:
              commitListItem.commit.author?.email || "unknown@example.com",
            date:
              commitListItem.commit.author?.date || new Date().toISOString(),
    ...
    const uniqueCommitsToStore = Array.from(
      new Map(
        allCommitsDataFromGitHub.map((commit) => [commit.sha, commit])
      ).values()
    );
    console.log(
      `[GitHubService] Total unique commits fetched for projectId ${projectId}: ${uniqueCommitsToStore.length}`
    );
    ...
    if (uniqueCommitsToStore.length > 0) {
      ...
      console.log(
        `[GitHubService] Initiated storing of ${uniqueCommitsToStore.length} commits for projectId ${projectId} via ContributionService.`
      );
    } else {
      console.log(
        `[GitHubService] No new commits to store for projectId ${projectId}.`
      );
    }
    ...
    return {
      ...
      count: uniqueCommitsToStore.length,
      data: uniqueCommitsToStore,
    };
  } catch (error) {
