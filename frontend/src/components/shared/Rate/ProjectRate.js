...
function ProjectRate() {
  ...
  const loadData = useCallback(async () => {
    //console.log("Params in ProjectRate:", {
    //  projectId,
    //  groupId: urlGroupId,
    //  currentUserId,
    //});
    ...
    if (
      ...
      setLoading(false);
      console.warn("Invalid params in ProjectRate:", {
        projectId,
        groupId: urlGroupId,
        currentUserId,
      });
  ...
  const renderStars = (memberId, field, value, isEditing) => {
    ...
    return stars.map((star) => (
      <span
        ...
        onClick={
          isEditing ? () => handleStarClick(memberId, field, star) : null
        }
  ...
  const handleSubmit = async (memberId) => {
    if (
      ...
      console.warn("Invalid params on submit:", {
        projectId,
        groupId: urlGroupId,
        currentUserId,
      });
            ...
            <div key={member.id} className="projectrate-task-item">
              <div className="projectrate-member-header">
                ...
                <div className="projectrate-member-stats">
                  <span>
                    TASK DONE: {taskStat.tasksCompleted}/
                    {taskStat.totalTasksAssigned}
                  </span>
              ...
              <div className="projectrate-note-section">
                ...
                {isEditing ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      handleNoteChange(member.id, e.target.value)
                    }
